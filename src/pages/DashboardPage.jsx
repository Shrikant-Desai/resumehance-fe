...
const DashboardPage = () => {
  ...
  const averageScore = analysisCount > 0
    ? Math.round(analyses.reduce((acc, curr) => acc + (curr.readiness_score || 0), 0) / analysisCount)
    : 0;
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-surface-container-low dark:bg-slate-900 rounded-2xl p-6 border border-slate-100/30 dark:border-slate-800 flex items-center justify-between">
          <div>
            ...
            <h3 className="text-3xl font-headline font-extrabold mt-2">{resumeCount}</h3>
          </div>
        ...
        <div className="bg-surface-container-low dark:bg-slate-900 rounded-2xl p-6 border border-slate-100/30 dark:border-slate-800 flex items-center justify-between">
          <div>
            ...
            <h3 className="text-3xl font-headline font-extrabold mt-2">{jobCount}</h3>
          </div>
          <div className="w-12 h-12 bg-indigo-50 dark:bg-slate-800 text-primary rounded-xl flex items-center justify-center">
        ...
        <div className="bg-surface-container-low dark:bg-slate-900 rounded-2xl p-6 border border-slate-100/30 dark:border-slate-800 flex items-center justify-between">
          <div>
            ...
            <h3 className="text-3xl font-headline font-extrabold mt-2">{analysisCount}</h3>
          </div>
          <div className="w-12 h-12 bg-indigo-50 dark:bg-slate-800 text-primary rounded-xl flex items-center justify-center">
        ...
        <div className="bg-surface-container-low dark:bg-slate-900 rounded-2xl p-6 border border-slate-100/30 dark:border-slate-800 flex items-center justify-between">
          <div>
            ...
            <h3 className="text-3xl font-headline font-extrabold mt-2 text-secondary">{averageScore}%</h3>
          </div>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-on-surface-variant mt-0.5">
                          Resume ID: {run.resume_id} • {run.created_at ? new Date(run.created_at).toLocaleDateString() : "Just now"}
                        </p>
                      </div>
                    </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-surface-container-low dark:bg-slate-900 border border-slate-100/30 dark:border-slate-800 rounded-2xl p-6 text-left">
            <h4 className="font-headline text-md font-bold mb-4">Quick Operations</h4>
            <div className="space-y-3">
              <button
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-on-surface-variant">Configure parsed AI fields</p>
                </div>
              </button>
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-on-surface-variant">Paste roles and priorities</p>
                </div>
              </button>
            </div>
          </div>
          ...
          <div className="bg-primary text-on-primary rounded-2xl p-6 text-left relative overflow-hidden shadow-lg shadow-indigo-600/10">
            <h4 className="font-headline text-md font-bold mb-2">Curator AI Coach</h4>
            ...
            <div className="absolute right-[-10px] bottom-[-10px] opacity-15">
              <span className="material-symbols-outlined text-8xl" style={{ fontVariationSettings: "'OPSZ' 48" }}>
                auto_awesome
              </span>
            </div>
          </div>
